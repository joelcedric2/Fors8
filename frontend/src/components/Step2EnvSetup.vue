    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Automatically invokes tools to extract entities and relations from the knowledge graph, initializes simulated individuals, and assigns unique behaviors and memories based on real-world seeds
          </p>
          ...
          <div v-if="profiles.length > 0" class="profiles-preview">
            <div class="preview-header">
              <span class="preview-title">{{ profiles[0]?.actor_type ? 'Geopolitical Actor Profiles' : 'Generated Agent Profiles' }}</span>
            </div>
            <div class="profiles-list">
              <!-- Geopolitical Actor Card -->
              <template v-if="profiles[0]?.actor_type">
                <div
                  v-for="(profile, idx) in profiles"
                  :key="idx"
                  class="profile-card geo-card"
                  :class="'tier-' + (profile.tier || 'operational')"
                  @click="selectProfile(profile)"
                >
                  <div class="profile-header">
                    <span class="profile-realname">{{ profile.actor_name || 'Unknown' }}</span>
                    <span class="profile-username">{{ profile.actor_type }}</span>
                  </div>
                  <div class="profile-meta">
                    <span class="profile-profession geo-tier">{{ (profile.tier || 'operational').toUpperCase() }}</span>
                    <span v-if="profile.nuclear_status && profile.nuclear_status !== 'none'" class="geo-nuclear">&#9762; {{ profile.nuclear_status }}</span>
                  </div>
                  <p class="profile-bio">{{ profile.description || profile.strategic_doctrine || 'No description' }}</p>
                  <div class="geo-params">
                    <span class="geo-param" title="Risk Tolerance">Risk: {{ ((profile.risk_tolerance || 0.5) * 100).toFixed(0) }}%</span>
                    <span class="geo-param" title="Force Strength">Force: {{ profile.initial_force_strength || 50 }}</span>
                    <span v-if="profile.martyrdom_willingness > 0" class="geo-param geo-ideology" title="Martyrdom Willingness">Martyrdom: {{ ((profile.martyrdom_willingness || 0) * 100).toFixed(0) }}%</span>
                  </div>
                  <div v-if="profile.alliance_network?.length" class="profile-topics">
                    <span
                      v-for="ally in (profile.alliance_network || []).slice(0, 3)"
                      :key="ally"
                      class="topic-tag geo-ally"
                    >{{ ally }}</span>
                  </div>
                </div>
              </template>
              <!-- Legacy Social Media Agent Card -->
              <template v-else>
                <div
                  v-for="(profile, idx) in profiles"
                  :key="idx"
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, individual activity windows, posting frequency, event triggers, and more based on simulation requirements and real-world seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, auto-generate initial activation events and trending topics to guide the simulation world's initial state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">Fors8 auto-plans simulation over <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>With 100 Agents: est. ~{{ Math.round(customMaxRounds * 0.6) }} min</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        With 100 Agents: est. {{ Math.round(autoGeneratedRounds * 0.6) }} min
                      </span>
                    </div>
                    <div class="auto-desc">
...
/* Geopolitical Actor Card Styles */
.geo-card {
  border-left: 3px solid #64748b;
}
.geo-card.tier-strategic {
  border-left-color: #ef4444;
}
.geo-card.tier-operational {
  border-left-color: #f97316;
}
.geo-card.tier-information {
